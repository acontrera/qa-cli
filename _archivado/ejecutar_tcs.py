...
def main():
    ...
    # Ejecutor OBLIGATORIO sin default + confirmacion
    print(f"\n[5/6] Quien ejecuta?")
    ejecutor = ""
    while True:
        nombre_temp = ""
        while not nombre_temp:
            ...
            if not nombre_temp:
                ...
                print(f"  [ERROR] El nombre parece muy corto. Escribe nombre y apellido.")
                nombre_temp = ""

        # Confirmacion para evitar errores de tecleo
        print(f"\n  Vas a registrar como ejecutor: >>> {nombre_temp} <<<")
        confirma = input(f"  Es correcto? (s/n): ").strip().lower()
        if confirma == "s":
            ejecutor = nombre_temp
            break
        print(f"  [INFO] Vuelve a escribir el nombre correctamente")
        print()
